...
public final class ImportScreen {
    ...
    public View render() {
        ...
        page.addView(currentLibraryPanel(), Views.matchWrap());
        page.addView(importButtonPanel(), Views.matchWrap());
        page.addView(Views.divider(context));

        ScrollView scroll = new ScrollView(context);
        LinearLayout list = Views.vertical(context);
        addSnapshotRows(list);
        scroll.addView(list);
    ...
    private View importButtonPanel() {
        LinearLayout panel = Views.vertical(context);
        panel.setPadding(0, 4, 0, 12);
        Button importButton = Views.button(context, UiText.choose(language, "导入新的 .db 商品库", "Importar nueva base .db"));
        importButton.setOnClickListener(v -> confirmImportNewDb());
        panel.addView(importButton, Views.matchWrap());

        TextView note = Views.text(
                context,
        ...
        panel.addView(note, Views.matchWrap());
        return panel;
    }
    ...
    private void confirmImportNewDb() {
        new AlertDialog.Builder(context)
                .setTitle(UiText.choose(language, "导入新的商品库？", "Importar nueva base?"))
                ...
                .setPositiveButton(UiText.choose(language, "继续导入", "Importar"), (dialog, which) -> importGateway.requestImportFile())
                .show();
    }
