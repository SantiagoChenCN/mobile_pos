...
更新时间：2026-07-19  
设计边界：docs/plans/ms2011_live_product_promotion_sync_plan.md
...
负责层：只读审计  
允许修改：仅新增任务明确授权的 workspace baseline 证据文档。
禁止修改：任何业务源码
...
- 合同 JSON、盘点 JSON 与退出码 TXT 保存在受控证据存储中；公开计划不保存临时传输链接。
...
2026-07-17 最终状态：EV-05 固定查询诊断器、定向/完整回归、`compileall`、独立安全复审、冻结打包和目标电脑取证均已完成。目标身份诊断结论为 `WRITE_CAPABILITY_PRESENT`；S01 证据收集完成，可继续离线 S02，但 EV-05 未达到 PASS，G0B 继续锁定。完整制品、hash 和原始证据位置只保留在受控历史证据记录中，不进入当前公开计划。
...
- 任务明确授权的 `<type>_results.md` 促销验证证据
...
若本机需要 ASCII-only 构建镜像，其路径只记录在忽略的 `.codex/local-workspace.md`；必须先同步本任务明确修改的文件并比较源码和产物 hash，不得把镜像当作新的开发主目录。
...
## 19. 通用 Subagent 提示词
    ...
    先阅读：
    1. docs/plans/ms2011_live_product_promotion_sync_plan.md
    2. docs/plans/ms2011_live_product_promotion_sync_implementation_plan.md
...
- docs/PROJECT_STATUS.md、docs/PROJECT_LOG.md 和 docs/plans/README.md 已按 DOC-01 同步。
