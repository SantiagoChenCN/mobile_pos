...
- 已还原三类复杂促销结构用于理解数据，但实操范围已确认只使用 `MS_GOODLIST.GHuiPrice + GHuiPriceCount` 简单促销。复杂促销表不进入手机端同步和价格计算，因此不存在简单/复杂促销叠加或优先级冲突。
- 分析复现脚本位于 `ESpsa_analysis/analyze_ms2011_export.py`，只读取导出的 TSV 和 EPSA 快照，不连接、不修改鸣盛实时数据库。

## 尚未完成

- 当前 `pc-sync-tool` 仍以鸣盛 `.db` 副本为同步源，尚未把 `MS2011` 的商品、分类、单位只读导出接入正式自动同步流程。
- 简单促销接入仍需验证：正常 `GHuiPrice/GHuiPriceCount` 门槛规则、`GHuiPriceCount<=0` 不触发优惠，以及 `GStopFlag=4` 始终优先于促销字段。
- 生产化实时同步仍需实现：专用只读连接/导出器、商品增量游标 `(GUpdateTime, GID)`、分类/单位全量快照哈希、快照原子替换和手机端导入适配；复杂促销表不纳入生产同步。
...
- 手机端只实现 `GHuiPrice + GHuiPriceCount` 简单促销；不读取、不传输、不计算复杂促销表，不做简单/复杂促销叠加。
...
1. 把 `MS_GOODLIST`、`MS_GOODTYPELIST`、`MS_UNITLIST` 接入电脑端只读提取和手机端商品快照，并随商品同步 `GHuiPrice/GHuiPriceCount` 简单促销字段；复杂促销表不接入。
2. 按 `(GUpdateTime, GID)` 实现商品增量同步，并为分类和单位实现全量快照哈希与原子替换。
3. 验证简单促销门槛、无效门槛不触发优惠和停用商品优先级，不再测试复杂促销优先级。
4. 在真实手机和收银电脑同一局域网下完成电脑端同步工具端到端联调。
5. 优化收银和商品编辑的关键词搜索体验，并按 `product_editing_plan.md` 完成真机验收。
6. 继续补齐销售/交易明细持久化、日账读取持久化销售仓库和按日期导出 CSV UI。
7. 后续再评估 `.xlsx` 商品导入和正式 release APK 签名。
